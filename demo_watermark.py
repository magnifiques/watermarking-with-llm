...
def generate_with_watermark_and_cluster(prompt, args, model=None, device=None, tokenizer=None):
    """Generate text with both watermark and cluster bias processors, and also return the output without watermark or bias."""
    
    print(f"Generating with watermark and cluster bias using {args}")
    ...
    # Initialize the cluster bias processor
    cluster_bias_processor = ClusterBiasLogitsProcessor(
        token_to_cluster=token_to_cluster,
        bias_strength=1.5  # Adjust bias strength as needed
    )

    # Combine the processors in a LogitsProcessorList for watermark and cluster bias
    logits_processors = LogitsProcessorList([watermark_processor, cluster_bias_processor])
    ...
    # Determine prompt max length based on model config or a default
    if args.prompt_max_length:
        pass
    ...
    # Tokenize input and handle potential truncation
    ...
    # Generate text without watermark or cluster bias
    generate_without_watermark = partial(model.generate, **gen_kwargs)
    output_without_watermark = generate_without_watermark(**tokd_input)

    # Optionally re-seed before generating with watermark and cluster bias
    if args.seed_separately: 
        torch.manual_seed(args.generation_seed)

    # Generate text with both watermark and cluster bias
    output_with_watermark_and_cluster = model.generate(
        **tokd_input, 
    ...
    if args.is_decoder_only_model:
        output_without_watermark = output_without_watermark[:, tokd_input["input_ids"].shape[-1]:]
        output_with_watermark_and_cluster = output_with_watermark_and_cluster[:, tokd_input["input_ids"].shape[-1]:]
    
    decoded_output_without_watermark = tokenizer.batch_decode(
        output_without_watermark, skip_special_tokens=True
    )[0]
    
    decoded_output_with_watermark_and_cluster = tokenizer.batch_decode(
        output_with_watermark_and_cluster, skip_special_tokens=True
    )[0]
    ...
    return (
        ...
        int(truncation_warning),
        decoded_output_without_watermark,
        decoded_output_with_watermark_and_cluster,
        args
    )
...
def run_gradio(args, model=None, device=None, tokenizer=None):
    ...
    if args.demo_public:
        demo.launch(concurrency_limit=10, server_name="0.0.0.0", server_port=8080, share='True') # exposes app to the internet via randomly generated link
    else:
        demo.launch(share='True')

def create_cluster(model, tokenizer, args):
    
    #? Step 1: Retrieve the embedding layer matrix
    # embedding_layer = model.get_input_embeddings()
    # token_embeddings = embedding_layer.weight  # Matrix of size (vocab size, embedding dimension)

    #? Step 2: Convert the embedding vectors to a NumPy array
    # token_vectors = token_embeddings.detach().cpu().numpy()  # Move to  CPU and convert to NumPy
    
    #? Step 3: Retrieve all valid token IDs and their respective token strings
    # token_id_to_token = {i: tokenizer.decode([i], clean_up_tokenization_spaces=True) for i in range(tokenizer.vocab_size)}
    
    #! Step 4: Combine the vectors and tokens into a DataFrame
    
    # df = pd.DataFrame(token_vectors[:tokenizer.vocab_size])  # Ensure the token vector count matches
    # df["token"] = [token_id_to_token[i] for i in range(tokenizer.vocab_size)]
    
    #? Step 5: Save the results to a CSV file with escape character set
    # df.to_csv(f"{args.model_name_or_path}_token_embeddings_with_tokens.csv", index=False, escapechar='\\')
    
    #? Step 6: Calculate adaptive `eps` value
    # Use NearestNeighbors to find the nearest neighbor distances for each point
    # neigh = NearestNeighbors(n_neighbors=10, metric='euclidean')
    # nbrs = neigh.fit(token_vectors[:tokenizer.vocab_size])
    # distances, _ = nbrs.kneighbors(token_vectors[:tokenizer.vocab_size])

    #?Step 7: Select the average distance to the 10th nearest neighbor as a reference
    # distances_to_kth_neighbor = distances[:, -1]
    # eps = np.percentile(distances_to_kth_neighbor, 90)  # 90th percentile distance as eps
    
    # print(f"Adaptive `eps` value calculated as: {eps}")

    #? Step 8: Perform DBHSCAN clustering
    
    # print("Perform DBSCAN clustering")
    # min_samples = 10  # Minimum number of points required per cluster
    
    # hdbscan_clusterer = hdbscan.HDBSCAN(min_samples=min_samples, min_cluster_size=10)
    # labels = hdbscan_clusterer.fit_predict(token_vectors[:tokenizer.vocab_size])
    # print('labels', labels)
    
    #? Step 9: Add clustering results to the DataFrame
    # print("Add clustering results to the DataFrame")
    # df['cluster'] = labels  # Add clustering labels to the original data

    #? Step 10: Save the results with clustering information to a CSV file
    # df.to_csv(f"{args.model_name_or_path}_token_embeddings_with_clusters.csv", index=False, escapechar='\\')
    ...
    token_to_cluster = {token_id: cluster_id for token_id, cluster_id in token_to_cluster.items() if cluster_id != -1}
    
    print(f"Token to cluster mapping created with {len(token_to_cluster)} tokens.")
